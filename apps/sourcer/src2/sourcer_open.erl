...
o_parse_record([#token{kind='('}, #token{value=Value, offset=O, length=L}, #token{kind=','} | _Tokens], Offset)
  when Offset=<O+L ->
    throw({open, {record, Value}});
o_parse_record([#token{kind='('}, #token{value=Value}, #token{kind=','} | Tokens], Offset) ->
    Between = sourcer_np_util:get_between_outer_pars(Tokens, '{', '}'),
    o_parse_record_aux(Between, Offset, Value, want_field).

o_parse_record_aux([], _Offset, Record, _) ->
    throw({open, {record, Record}});
o_parse_record_aux([#token{offset=O, length=L, value=Field} | _], Offset, Record, want_field) when Offset<O+L ->
    throw({open, {field, Record, Field}});
o_parse_record_aux([#token{value=V} | _]=Tokens, Offset, Record, _) when V=:='('; V=:='{'; V=:='['; V=:='<<' ->
    Rest = sourcer_text:skip_expr(Tokens),
    o_parse_record_aux(Rest, Offset, Record, want_comma);
o_parse_record_aux([#token{value=','} | Rest], Offset, Record, want_comma) ->
    o_parse_record_aux(Rest, Offset, Record, want_field);
o_parse_record_aux([_ | Rest], Offset, Record, W) ->
    o_parse_record_aux(Rest, Offset, Record, W).
