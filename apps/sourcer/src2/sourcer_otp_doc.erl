...
get_proposals(Mod0, Prefix, StateDir) ->
    {Mod, Functions} = case Mod0 of
                           '<auto_imported>' ->
                               {erlang, sourcer_util:get_auto_imported(Prefix)};
                           _ ->
...
extract_pars(FunctionName, Arity, Offset, Doc) ->
    Sub1 = sourcer_util:get_all_between_strs(Doc, "<CODE>", "</CODE>"),
    Sub2 = sourcer_util:get_all_between_strs(Doc, "<a name =", "</span>"),
    Sub3 = sourcer_util:get_all_between_strs(Doc, "<a name=", "</span>"),
